...
async def bulk_onboard(
    ...
    """
    Parse a two-tab bulk onboarding XLSX and return structured data.
    No DB writes — the response populates the manual review form.
    The user reviews and publishes via POST /multi-publish.
    """
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file provided")
    data = await file.read()
    try:
        contract_meta, line_items = _parse_bulk_two_tab(data)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))

    if not contract_meta.get("vendor_name") and not contract_meta.get("po_number"):
        raise HTTPException(
            status_code=400,
            detail="Contract Information tab is empty — fill in at least Vendor Name and PO Number",
        )

    return {
        "vendor_name": contract_meta.get("vendor_name"),
        "po_number": contract_meta.get("po_number"),
        "contract_name": contract_meta.get("contract_name"),
        "start_date": contract_meta.get("start_date"),
        "end_date": contract_meta.get("end_date"),
        "clm_id": contract_meta.get("clm_id"),
        "auto_renewal_clause": contract_meta.get("auto_renewal_clause"),
        "currency": contract_meta.get("currency", "INR"),
        "line_items": [
            {
                "contract_name": contract_meta.get("contract_name"),
                "primary_sw_name": li["primary_sw_name"],
                "sw_id": li["sw_id"],
                "license_type": li["license_type_name"],
                "metric_name": li["metric_name"],
                ...
                "sub_category_name": li["sub_category_name"],
                "gxp_flag": li["gxp_flag"],
                "vendor_risk": li["vendor_risk"],
                "deployment": li["deployment"],
                "notes": li["notes"],
                "app_owner_email": li["app_owner_email"],
                "secondary_owner_email": li["secondary_owner_email"],
                "doa_emails": li["doa_emails"],
                "discovery_source_name": li["discovery_source_name"],
                "usage_method_name": li["usage_method_name"],
            }
